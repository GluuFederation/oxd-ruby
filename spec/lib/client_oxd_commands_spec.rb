...
describe ClientOxdCommands do
	...
	describe "#get_authorization_url" do
		it 'returns authorization_url' do
		    ...
		    expect(authorization_url).to match(/client_secret/)
		end
	...
	describe "#get_tokens_by_code" do
		it 'returns access_token' do
			...
			access_token = client_oxd_cmd.get_tokens_by_code(code, scopes, state)
			expect(access_token).to eq("mock-token")
		end

		it 'raises error for invalid arguments' do
			# Empty code should raise error
			expect{ @oxd_command.get_tokens_by_code("", ["openid"], "state") }.to raise_error(RuntimeError)

		    # Empty array for scopes should raise error
		    expect{ @oxd_command.get_tokens_by_code("code", [], "state") }.to raise_error(RuntimeError)

		    # raise error when scopes is not an array
		    expect{ @oxd_command.get_tokens_by_code("code", "openid", "state") }.to raise_error(RuntimeError)
		end
		...
		it 'raises error if response has error' do
		    ...
		    state = "af0ifjsldkj"
		    scopes = ["openid","profile"]		    
			expect{ @oxd_command.get_tokens_by_code(code, scopes, state) }.to raise_error(RuntimeError)
		end
	end
